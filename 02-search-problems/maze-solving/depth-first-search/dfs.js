...
class BuscaEmProfundidade {  
    ...
    static executar(lab) {
        ...
        // inicio a pilha de pesquisa com o ponto inicial
        pilhaPesquisa.push(newLab.posi);
        // registro das posições que já foram verificadas
        let posicoesVerificadas = new Map();
        let pm = []; // proximos movimentos
        while (pilhaPesquisa.length > 0) {
            // retiro o elemento da pilha
            let pos = pilhaPesquisa.pop();
            // cheguei no alvo?
            if (newLab.noAlvo(pos)) {
                return pos;
            } else { // ainda não cheguei no alvo
                if (!posicoesVerificadas.has(pos.getKey())) {
                    posicoesVerificadas.set(pos.getKey());
                    pm = newLab.proximosMovimentos(pos);
                    for (let i = 0; i < pm.length; i++) {
                        pilhaPesquisa.push(pm[i]);
                    }
                }
            }
        } // while
...
const pi = lab4.posi;
BuscaEmProfundidade.setCaminho(lab4);
...
*/
/*
let lab5 = new Labirinto(20, 50, new Pos(19, 49), new Pos(0, 0), 0.30);
console.log('ANTES DE SETAR O CAMINHO:');
lab5.visualizar();
console.log('DEPOIS DE SETAR O CAMINHO:');
const pi = lab5.posi;
BuscaEmProfundidade.setCaminho(lab5);
